...
$wporg_url = isset($options['wporg_url']) ? $options['wporg_url'] : '#';
...
<section class="min-h-screen flex items-center pt-20 pb-12 px-6 md:px-12 max-w-7xl mx-auto">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
        ...
        <div class="lg:col-span-5 flex justify-center lg:block animate-fade-in-right">
            <div class="relative group mx-auto lg:ml-auto lg:mr-0 w-fit">
                <!-- Decorative Shapes -->
                <div class="absolute -inset-4 border-2 border-primary/20 rounded-2xl -z-10 group-hover:border-primary/40 transition-colors duration-500 animate-float-slow"></div>
                <div class="absolute -inset-8 border border-slate-700/50 rounded-2xl -z-20 group-hover:border-slate-600 transition-colors duration-500 animate-float-slower"></div>

                <!-- Main Image Container -->
                <div class="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 rounded-2xl overflow-hidden glass-panel border border-slate-700 p-2 group-hover:border-primary/50 transition-all duration-500 shadow-2xl">
                    <div class="w-full h-full rounded-xl overflow-hidden relative">
                        <img src="<?php echo get_template_directory_uri(); ?>/assets/image/profile-image.png" alt="<?php echo esc_attr($hero_title); ?>" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                        <!-- Overlay Gradient -->
                ...
                <!-- Floating Tech Badge -->
                <div class="absolute -bottom-6 -left-6 bg-slate-900/90 backdrop-blur-xl border border-primary/30 p-4 rounded-xl shadow-2xl animate-float font-mono text-xs text-primary">
    ...
    <section id="about" class="py-24 px-6 md:px-12 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div class="space-y-6 text-slate-400 text-lg leading-relaxed reveal">
                ...
                <div class="pt-4 flex flex-wrap gap-6 font-mono text-sm">
                    <?php if ($linkedin_url) : ?>
                    ...
                    <?php endif; ?>
                    <?php if ($wporg_url) : ?>
                        <a href="<?php echo esc_url($wporg_url); ?>" target="_blank" class="text-slate-400 hover:text-primary transition-colors flex items-center gap-2">
                            <span class="text-2xl">&#127760;</span> WordPress
                        </a>
                    <?php endif; ?>
                    <?php if ($facebook_url) : ?>
                        <a href="<?php echo esc_url($facebook_url); ?>" target="_blank" class="text-slate-400 hover:text-primary transition-colors flex items-center gap-2">
...
<span class="text-pink-400">const</span> <span class="text-blue-400">developer</span> = {
  <span class="text-primary">name:</span> <span class="text-green-400">'Sydur Rahman'</span>,
  <span class="text-primary">role:</span> <span class="text-green-400">'Full Stack Web Developer'</span>,
  <span class="text-primary">base:</span> <span class="text-green-400">'Dhaka, Bangladesh'</span>,
  <span class="text-primary">uniqueTrait:</span> <span class="text-green-400">'MBA in Accounting'</span>,
  <span class="text-primary">interests:</span> [
    <span class="text-green-400">'Scalable Architecture'</span>,
    <span class="text-green-400">'Reactive UI (Vue.js)'</span>,
    <span class="text-green-400">'Legacy Code Rescue'</span>
  ],
...
<?php if (empty($options['hide_services'])) : ?>
    <section id="services" class="py-24 px-6 md:px-12 max-w-7xl mx-auto relative overflow-hidden">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            ?>
                    <?php
                    $custom_icon = get_post_meta(get_the_ID(), 'service_icon', true) ?: '&#9881;';
                    ?>
                    <a href="<?php the_permalink(); ?>" class="glass-panel p-10 rounded-3xl reveal hover:-translate-y-2 transition-all duration-500 group border border-slate-700/50 hover:border-primary/40 flex flex-col h-full" style="transition-delay: <?php echo esc_attr($delay); ?>ms;">
                        <div class="flex justify-between items-start mb-8">
                            <div class="text-4xl text-slate-400 group-hover:text-primary transition-all transform group-hover:rotate-12 duration-500"><?php echo $custom_icon; ?></div>
                            <span class="text-[10px] font-mono text-slate-400 uppercase tracking-widest bg-slate-800/80 px-3 py-1 rounded-full border border-slate-700 group-hover:border-primary/30 transition-colors">Service</span>
                        </div>
...
<?php if (empty($options['hide_contact'])) : ?>
    <section id="contact" class="py-32 px-6 md:px-12 max-w-4xl mx-auto text-center reveal">
        ...
        <p class="text-lg text-slate-400 mb-10 leading-relaxed max-w-2xl mx-auto">
            <?php echo wp_kses_post(isset($options['contact_description']) && !empty($options['contact_description']) ? $options['contact_description'] : "I'm currently open for new opportunities. Whether you have a question or just want to say hi, I'll try my best to get back to you!"); ?>
        </p>
        <div class="max-w-xl mx-auto">
            <?php
            $main_form = isset($options['main_contact_shortcode']) ? $options['main_contact_shortcode'] : '';
            if (! empty($main_form)) :
                echo do_shortcode($main_form);
            else : ?>
                <!-- Built-in Contact Form Fallback -->
                <form action="#" class="space-y-4 text-left glass-panel p-8 rounded-3xl border border-slate-700/50">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block font-mono text-xs text-slate-500 mb-2 uppercase tracking-widest">Name</label>
                            <input type="text" placeholder="Your Name" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
                        </div>
                        <div>
                            <label class="block font-mono text-xs text-slate-500 mb-2 uppercase tracking-widest">Email</label>
                            <input type="email" placeholder="hello@example.com" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block font-mono text-xs text-slate-500 mb-2 uppercase tracking-widest">Message</label>
                        <textarea rows="4" placeholder="How can I help you?" class="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"></textarea>
                    </div>
                    <button type="submit" class="w-full py-4 bg-primary text-slate-900 font-bold rounded-xl hover:bg-primaryHover transition-all transform hover:-translate-y-1 shadow-lg shadow-primary/20 uppercase tracking-widest text-sm">
                        Send Message
                    </button>
                    <p class="text-center text-[10px] text-slate-500 font-mono mt-4">
                        Or email directly at <a href="mailto:<?php echo esc_attr($contact_email); ?>" class="text-primary hover:underline"><?php echo esc_html($contact_email); ?></a>
                    </p>
                </form>
            <?php endif; ?>
        </div>
