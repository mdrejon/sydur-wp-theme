...
<body <?php body_class('font-sans antialiased relative selection:bg-primary selection:text-slate-900'); ?>>
    ...
    <!-- Navigation -->
    <nav id="navbar" class="fixed top-0 left-0 w-full z-50 transition-all duration-300 py-4 <?php echo is_front_page() ? '' : 'glass-panel shadow-md '; ?>">
        <div class="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center w-full">
            <a href="<?php echo esc_url(home_url('/')); ?>" class="text-2xl font-bold text-white tracking-tighter flex items-center gap-1 group flex-shrink-0">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="hidden lg:flex items-center font-mono text-sm primary-menu-container">
                    ...
                    <a
                        ...
                        <?php echo $btn_target; ?>><?php echo esc_html($btn_label); ?></a>
                </div>

                <!-- Mobile Menu Button -->
                <button class="lg:hidden text-white hover:text-primary text-3xl leading-none transition-colors" id="mobile-menu-btn" aria-label="Menu">
                    &#9776;
                </button>
            </div>
        </div>
